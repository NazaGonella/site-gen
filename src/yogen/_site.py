...
class Site:
    def __init__(self, content_path : str, build_path : str, static_path : str, templates_path : str, rss_config_path : str):
        self.content_path : Path = Path(content_path)
        self.build_path : Path = Path(build_path)
        self.static_path : Path = Path(static_path)             # contents will be copied inside the build folder on build
    ...
    def build(self):
        ...
        # static files
        for file in self.static_path.rglob("*"):
            target = self.build_path / file.relative_to(self.static_path)
            if file.is_dir():
